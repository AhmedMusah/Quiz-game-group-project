...
int check_answer(char user_ans, char correct_ans)
{
    if (toupper(user_ans) == correct_ans)
    {
        printf(" Correct!\n");
        return 1;
    }
...
int run_loop(TriviaQuestion q[], int n)
{
    ...
    for (int j = 1; j <= n;)
    {
        ...
        if (!used_questions[i])
        {
            // Reset shared timer state for each question
            time_up = 0;
            answered = 0;
            ...
            if (time_up)
            {
                printf(" Too slow! The correct answer was %c.\n", q[i].answer);
            }
            else
            {
...
int quiz_user(char c, char d, int n)
{
    ...
    case 'E': // English
        switch (d)
        {
        case 'E': score = run_loop(easy_eng, n);  break;
        case 'M': score = run_loop(med_eng, n);   break;
        case 'H': score = run_loop(hard_eng, n);  break;
        }
        break;
    ...
    case 'M': // Math
        switch (d)
        {
        case 'E': score = run_loop(easy_math, n); break;
        case 'M': score = run_loop(med_math, n);  break;
        case 'H': score = run_loop(hard_math, n); break;
        }
        break;
    ...
    case 'S': // Science
        switch (d)
        {
        case 'E': score = run_loop(easy_sci, n);  break;
        case 'M': score = run_loop(med_sci, n);   break;
        case 'H': score = run_loop(hard_sci, n);  break;
        }
        break;
    ...
    case 'H': // History
        switch (d)
        {
        case 'E': score = run_loop(easy_hist, n); break;
        case 'M': score = run_loop(med_hist, n);  break;
        case 'H': score = run_loop(hard_hist, n); break;
        }
        break;
...
int main()
{
    ...
    for (;;)
    {
        ...
        do
        {
            printf("\nSelect Subject (E=English, M=Math, S=Science, H=History): ");
        ...
        // 4. Run quiz
        printf("\n--- Starting Quiz ---\n");
        int score = quiz_user(subject, difficulty, number_of_questions);
        printf("\n==============================\n");
